...
@triton.jit()
def _legacy_replay_precompute_kernel(
    # Input pointers
    dt_ptr,
    dt_bias_ptr,
    A_ptr,
...
@triton.heuristics({"BLOCK_SIZE_T": lambda args: max(triton.next_power_of_2(args["T"]), 16)})
@triton.jit()
def _legacy_replay_state_update_kernel(
    # Pointers
    state_ptr,
...
def legacy_replay_selective_state_update(
    state: torch.Tensor,
    old_x: torch.Tensor,
    old_B: torch.Tensor,
    old_dt: torch.Tensor,
    ...
    with torch.cuda.device(device.index):
        ...
        _legacy_replay_precompute_kernel[(batch, nheads // heads_per_block)](
            dt,
            dt_bias,
            A,
            B,
        ...
        _legacy_replay_state_update_kernel[grid](
            state,
            old_x,
            old_B,
            old_dt,
